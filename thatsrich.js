...
while ((match = regexp.exec(textarea.value)) !== null) {
  ...
  textarea.value.slice(tableStart, regexp.lastIndex - 2).split(/^ *\|-/gm).forEach(function (itm, rowIdx) {
      rows[rowIdx] = 1;
      cells[rowIdx] = [];
      content[rowIdx] = [];
  ...
  var separator = Array(indent + 1).join(' ') + '+' + cols.map(function (itm) { return Array(itm + 3).join('-') }).join('+') + '+';

  value += textarea.value.slice(end, tableStart) + separator + '\n' + content.map(function (itm) { return Array(indent + 1).join(' ') + '| ' + itm.map(function (itm, idx) { return itm + Array(cols[idx] - itm.length + 1).join(' ') }).join(' | ') + ' |\n' }).join(separator + '\n') + separator;
